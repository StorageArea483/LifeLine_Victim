...
import 'package:life_line_victim/providers/chat_bot_provider.dart';

import 'package:life_line_victim/styles/styles.dart';
import 'package:life_line_victim/models/message.dart';
import 'package:life_line_victim/services/severity_service.dart';
...
class _ChatBotState extends ConsumerState<ChatBot> {
  ...
  Future<void> _initializeSpeech() async {
    try {
      _speechInitialized = await _speechToText.initialize(
        onError: (error) {
          if (mounted) {
            ref.read(chatPageProvider.notifier).setIsSpeechListening(true);
        ...
        onStatus: (status) {
          if (status == 'done' || status == 'notListening') {
            if (mounted) {
              ref.read(chatPageProvider.notifier).setIsSpeechListening(false);
            }
          }
        },
      );
  ...
  void _handleExceptionError(String msg) {
    pageMessage(msg, context, AppColors.error);
  }
  ...
  Future<void> _checkAndUpdateSeverity() async {
    ...
    if (currentStep > answerOptions.length - 1) {
      try {
        ...
        if (mounted) {
          ref.read(chatPageProvider.notifier).setIsSeverityUpdating(true);
        }
        ...
        // Extract report details from AI's last message
        final reportDetails = SeverityService.extractReportDetails(
          _conversationHistory,
          currentRequest,
        );

        // Update victim report in database
        await SeverityService.updateVictimReport(reportDetails);

        _severityUpdated = true;

        // Set loading state to false
        if (mounted) {
          ref.read(chatPageProvider.notifier).setIsSeverityUpdating(false);
          ref.invalidate(chatPageProvider);
        }

        // Show NGO connection sheet
        if (mounted) {
          NgoConnectSheet.show(
            context,
            requestType: widget.request,
  ...
  bool _isTextFieldEnabled() {
    ...
    final currentRequest = widget.request;
    // Enable for medical requests, disable for flood/earthquake
    return currentRequest == null || currentRequest.toLowerCase() == 'medical';
  }
    ...
    return Scaffold(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListTile(
                            ...
                            title: Text(
                              'Clear all messages',
                              style: AppText.small.copyWith(color: Colors.red),
                            ),
                            onTap: () {
                              if (mounted) {
      ...
      body: Stack(
        children: [
          ...
          Consumer(
            builder: (context, ref, child) {
              final isSeverityLoading = ref.watch(
                chatPageProvider.select((v) => v.isSeverityUpdating),
              );

              if (!isSeverityLoading) return const SizedBox.shrink();
              ...
              Flexible(
                child: Container(
                  ...
                  constraints: BoxConstraints(
                    maxWidth: ResponsiveHelper.messageWidth(context),
                  ),
                  decoration: BoxDecoration(
                    color:
                        message.isUser
      ...
      child: SafeArea(
        child: Center(
          child: ConstrainedBox(
            constraints: BoxConstraints(
              maxWidth: ResponsiveHelper.contentWidth(context),
            ),
            child: Row(
              children: [
                Expanded(
                  child: Container(
                    ...
                    child: Consumer(
                      builder: (context, ref, child) {
                        ...
                        final enabled = _isTextFieldEnabled();
                        return TextField(
                        ...
                        Consumer(
                          builder: (context, ref, _) {
                            final isListening = ref.watch(
                              chatPageProvider.select(
                                (v) => v.isSpeechListening,
                              ),
                            );
                            return Container(
                              decoration: BoxDecoration(
                                color:
                                    isListening
                                        ...
                                        : enabled
                                        ? AppColors.primaryMaroon
                                        : AppColors.primaryMaroon.withOpacity(
                                          0.6,
                                        ),
